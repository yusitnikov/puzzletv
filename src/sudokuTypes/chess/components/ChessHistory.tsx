...
export const ChessHistory = observer(function ChessHistory({context}: ChessHistoryProps) {
    const moves = useChessHistory(context, false);

    return <RulesParagraph>
        {splitArrayIntoChunks(moves, 2).map((pair, index) => <div key={index}>
            {index + 1}.
            &nbsp;
...
export const useChessHistory = (context: PuzzleContext<ChessPTM>, untilCurrentState: boolean) => {
    const {fieldStateHistory, puzzle} = context;

    return useMemo(() => {
        let states = fieldStateHistory.states.map(
            (state) => unserializeFieldState(JSON.parse(state), puzzle)
                .cells
        ...
        for (let i = 0; i < states.length - 1; i++) {
            ...
            const changes: Partial<Record<ChessColor, { captured?: ChessPiece, movesMap: Partial<Record<ChessPieceType, Partial<Line>>> }>> = {};
        ...
        return moves;
    }, [fieldStateHistory, untilCurrentState, puzzle]);
};
