...
export const getChessMoveDescription = ({piece: {type}, capturedPiece, promotionPiece, start, end}: ChessMove, short: boolean) =>
    type === ChessPieceType.king && Math.abs(start.left - end.left) === 2
        ? end.left > start.left ? "0-0" : "0-0-0"
        : [
            type !== ChessPieceType.pawn && ChessPieceTypeNotation[type],
            !short ? stringifyChessCell(start) : (capturedPiece && type === ChessPieceType.pawn) ? stringifyChessCell(start)[0] : "",
            capturedPiece && "x",
