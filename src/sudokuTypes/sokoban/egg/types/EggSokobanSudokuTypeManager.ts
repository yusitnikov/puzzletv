...
const isImportedEgg = (item: Constraint<SokobanPTM, any>): item is Constraint<SokobanPTM, KillerCageProps> => isCageConstraint(item)
    && item.cells.length === 1
    ...
    postProcessPuzzle(puzzle): PuzzleDefinition<SokobanPTM> {
        ...
        puzzle = {
            ...
            items: [
                ...
                ...items.filter(isImportedEgg).map(
                    ({cells: [cell], props: {lineColor}}) => EggConstraint(cell, lineColor)
                ),
            ],
        };
