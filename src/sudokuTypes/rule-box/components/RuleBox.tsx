...
export const RuleBox: ConstraintPropsGenericFcMap<RuleBoxProps, AnyRuleBoxPTM> = {
    [FieldLayer.regular]: observer(function RuleBoxUi<T extends AnyRuleBoxPTM>({
        ...
        color = lightRedColor,
        context,
    }: ConstraintProps<T, RuleBoxProps>) {
        profiler.trace();
        ...
        return (
            <AutoSvg {...bounds}>
                <Gift width={bounds.width} height={bounds.height} color={color} />
            </AutoSvg>
        );
    }),
    [FieldLayer.interactive]: observer(function RuleBoxClicker<T extends AnyRuleBoxPTM>({
        ...
        const { fogVisibleCells, cellSizeForSidePanel: cellSize } = context;
        ...
        return (
            <>
                {!(context.stateExtension as RuleBoxGameState).ruleBoxes[rulesText] && (
                    <StyledClickArea
                        onClick={() => {
                            setShowModal(true);
                    ...
                    >
                        {cells.map(
                            ({ top, left }) =>
                                (!fogVisibleCells || fogVisibleCells[top][left]) && (
                                    <rect
                                        key={`${top}-${left}`}
                                        fill={"none"}
                                        stroke={"none"}
...
export const RuleBoxConstraint = <T extends AnyRuleBoxPTM>(
    cells: PositionLiteral[],
    rulesText: string,
    rulesNode: ReactNode,
): Constraint<T, RuleBoxProps> => ({
    name: "rules box",
    tags: [ruleBoxTag],
...
export const isRuleBoxConstraint = <T extends AnyRuleBoxPTM>(
    constraint: Constraint<T, any>,
): constraint is Constraint<T, RuleBoxProps> => (constraint.tags ?? []).includes(ruleBoxTag);
