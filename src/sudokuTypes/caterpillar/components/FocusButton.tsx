...
const focusNextGrid = <T extends AnyPTM>(context: PuzzleContext<CaterpillarPTM<T>>, fieldRect: Rect) => {
    ...
    const nextGrid = caterpillarGrids.find(
        (grid) => getGridRegionCells(grid.bounds).some(
            ({top, left}) => context.getUserDigit(top, left) === undefined
        )
    ...
    const nextSelectedCell = getGridRegionCells(nextGrid.bounds).find(
        (cell) => !caterpillarGrids.some(
            (grid) => grid !== nextGrid && isCellInRect(grid.bounds, cell)
        )
    );
    if (nextSelectedCell) {
        context.onStateChange((context) => gameStateSetSelectedCells(
            context,
            [nextSelectedCell],
        ));
    }
...
const FocusButton = observer(function FocusButtonComponent<T extends AnyPTM>({context, top, left}: ControlButtonItemProps<CaterpillarPTM<T>>) {
    profiler.trace();
...
export const FocusButtonRule = observer(function FocusButtonRule<T extends AnyPTM>({context}: FocusButtonRuleProps<T>) {
    profiler.trace();
