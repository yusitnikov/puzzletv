...
const svgMap: Record<number, ReactElement> = {
    1: <rect x={-width / 2} y={-0.5} width={width} height={1} fill={"currentColor"} strokeWidth={0} />,
    2: (
        <>
            <rect x={-0.15 - width / 2} y={-0.5} width={width} height={1} fill={"currentColor"} strokeWidth={0} />
            <rect x={0.15 - width / 2} y={-0.5} width={width} height={1} fill={"currentColor"} strokeWidth={0} />
        </>
    ),
    3: (
        <>
            <rect x={-0.3} y={-0.5} width={width} height={1} fill={"currentColor"} strokeWidth={0} />
            <rect x={-width / 2} y={-0.5} width={width} height={1} fill={"currentColor"} strokeWidth={0} />
            <rect x={0.3 - width} y={-0.5} width={width} height={1} fill={"currentColor"} strokeWidth={0} />
        </>
    ),
    4: (
        <>
...
export const LatinDigitSvgContent: DigitPropsGenericFc = observer(function LatinDigitSvgContentFc<T extends AnyPTM>({
    digit,
    size,
    color,
    left = 0,
    ...
    return (
        <g transform={`translate(${left} ${top}) scale(${size * 0.7})`} color={color}>
            <rect x={-width / 2} y={-0.5 - height} width={width} height={height / 2} fill={color} strokeWidth={0} />
            {svgMap[digit]}
            <rect x={-width / 2} y={0.5 + height / 2} width={width} height={height / 2} fill={color} strokeWidth={0} />
        </g>
    );
});
