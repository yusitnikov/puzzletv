...
export const FieldCellsLayer = observer(function FieldCellsLayer<T extends AnyPTM>(
    {context, topOffset, leftOffset, children, region, isInteractionMode = false}: FieldCellsLayerProps<T>
) {
    profiler.trace();
    ...
    return <>
        {indexes(fieldSize.rowsCount).flatMap((rowIndex) => indexes(fieldSize.columnsCount).map((columnIndex) => {
