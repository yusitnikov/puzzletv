...
export const FieldLoop = observer(function FieldLoop<T extends AnyPTM>({context: {puzzle}, children}: FieldLoopProps<T>) {
    profiler.trace();
...
const FieldLoopItem = observer(function FieldLoopItem<T extends AnyPTM>({topOffset, leftOffset, children}: FieldLoopItemProps<T>) {
    profiler.trace();

    return <>
        {
            typeof children === "function"
                ? children(topOffset, leftOffset)
                : children
        }
    </>;
