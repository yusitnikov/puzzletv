...
export const FieldItems = observer(function FieldItems<T extends AnyPTM>(props: FieldItemsProps<T>) {
    ...
    return <FieldLoop context={props.context}>
...
const FieldItemsInner = observer(function FieldItemsInner<T extends AnyPTM>(
    {context, layer, region, regionIndex}: FieldItemsProps<T>
) {
    profiler.trace();

    return <>
        {context.getVisibleItemsForLayer(layer).map(({component, cells, renderSingleCellInUserArea, ...otherData}, index) => {
