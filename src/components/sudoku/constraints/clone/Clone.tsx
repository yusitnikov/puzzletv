...
export const Clone: ConstraintPropsGenericFcMap = {
    [FieldLayer.beforeSelection]: observer(function Clone<T extends AnyPTM>({
        ...
        return (
            <>
                {cells.map(({ top, left }, index) => (
                    <rect key={index} x={left} y={top} width={1} height={1} fill={color} />
                ))}
            </>
        );
    }),
...
export const CloneConstraint = <T extends AnyPTM>(cellLiterals: PositionLiteral[], color?: string): Constraint<T> => ({
    ...
    isValidCell(cell, digits, cells, context) {
        ...
        const cell2 = incrementArrayItem(cells, (position) => isSamePosition(cell, position), cells.length / 2);
