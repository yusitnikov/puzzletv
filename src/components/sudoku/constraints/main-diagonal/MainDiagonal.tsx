...
export const DiagonalComponent: ConstraintPropsGenericFcMap<LineProps> = {
    [FieldLayer.regular]: observer(function Diagonal<T extends AnyPTM>(
        ...
        return <path
            d={[
                "M", x1, y1 + widthOffset.top,
                "v", -widthOffset.top,
                "h", widthOffset.left,
                "L", x2, y2 - widthOffset.top,
                "v", widthOffset.top,
                "h", -widthOffset.left,
                "z"
            ].join(" ")}
...
export const PositiveDiagonalConstraint = <T extends AnyPTM>(fieldSize: number, color?: string, width?: number): Constraint<T, LineProps> => ({
    ...BaseDiagonalConstraint<T>(
        [{top: fieldSize - 1, left: 0}, {top: 0, left: fieldSize - 1}],
        color,
        width,
    ),
...
export const NegativeDiagonalConstraint = <T extends AnyPTM>(fieldSize: number, color?: string, width?: number): Constraint<T, LineProps> => ({
    ...BaseDiagonalConstraint<T>(
        [{top: 0, left: 0}, {top: fieldSize - 1, left: fieldSize - 1}],
        color,
        width,
    ),
