...
export const Even: ConstraintPropsGenericFcMap<EvenProps> = {
    [FieldLayer.beforeSelection]: observer(function Even<T extends AnyPTM>({
        cells: [{ left, top }],
        color = rgba(darkGreyColor, 0.6),
        props: { size = 0.8 },
    }: ConstraintProps<T, EvenProps>) {
        profiler.trace();
...
export const EvenConstraint = <T extends AnyPTM>(
    cellLiteral: PositionLiteral,
    color?: string,
    size?: number,
): Constraint<T, EvenProps> => {
