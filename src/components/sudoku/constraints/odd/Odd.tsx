...
export const Odd: ConstraintPropsGenericFcMap<OddProps> = {
    [GridLayer.beforeSelection]: observer(function Odd<T extends AnyPTM>({
        ...
        props: { size = 0.8 },
    }: ConstraintProps<T, OddProps>) {
        profiler.trace();

        const { rgb, a } = parseColorWithOpacity(color);

        return <circle cx={left + 0.5} cy={top + 0.5} r={size / 2} fill={rgb} opacity={a} />;
    }),
};

export const OddConstraint = <T extends AnyPTM>(
    cellLiteral: PositionLiteral,
    color?: string,
    size?: number,
): Constraint<T, OddProps> => {
