...
const NumberClue = observer(function NumberClueFc({ cell }: CellProps) {
    ...
    return (
        <CenteredText left={left + 0.25} top={top + 0.55} fill={sheetColor} size={0.25}>
            №
        </CenteredText>
    );
});
...
const KClue = observer(function KClueFc({ cell }: CellProps) {
    ...
    return (
        <CenteredText left={left + 0.75} top={top + 0.55} fill={sheetColor} size={0.25}>
            K
        </CenteredText>
    );
});
...
const Extras = {
    [GridLayer.afterLines]: observer(function Extras() {
        ...
        return (
            <>
                <CenteredText top={1.5} left={1} size={0.4} fill={sheetColor}>
                    Allegro
                </CenteredText>
                <CenteredText top={6.4} left={0.5} size={0.4} fill={sheetColor} style={{ fontStyle: "italic" }}>
                    p
                </CenteredText>
...
const CardImage = ({ onClick, style }: CardImageProps) => (
    <img
