...
const GlyphO = (kx: number, ky: number) => <path
    key={`o:${kx}:${ky}`}
    fill={"currentColor"}
    stroke={"none"}
    strokeWidth={0}
    d={[
        "M", kx * 0.5, ky * smallWidth / 2,
        "Q", kx * largeWidth / 2, ky * smallWidth / 2, kx * largeWidth / 2, ky * 0.5,
        "H", -kx * largeWidth / 2,
        "Q", -kx * largeWidth / 2, -ky * smallWidth / 2, kx * 0.5, -ky * smallWidth / 2,
        "z",
    ].join(" ")}
/>;
...
const GlyphSmallEnding = (k: number) => <path
    key={`small-ending:${k}`}
    fill={"currentColor"}
    stroke={"none"}
    strokeWidth={0}
    d={[
        "M", k * 0.5, smallWidth / 2,
        "q", -k * smallCurve, 0, -k * smallCurve, smallCurve,
        "h", -k * smallWidth,
        "v", -smallWidth - 2 * smallCurve,
        "h", k * smallWidth,
        "q", 0, smallCurve, k * smallCurve, smallCurve,
        "z",
    ].join(" ")}
/>;
...
const GlyphLargeEnding = (k: number) => <path
    key={`large-ending:${k}`}
    fill={"currentColor"}
    stroke={"none"}
    strokeWidth={0}
    d={[
        "M", largeWidth / 2, k * 0.5,
        "q", 0, -k * largeCurve, largeCurve, -k * largeCurve,
        "v", -k * smallWidth,
        "h", -largeWidth - 2 * largeCurve,
        "v", k * smallWidth,
        "q", largeCurve, 0, largeCurve, k * largeCurve,
        "z",
    ].join(" ")}
/>;
...
const GlyphVerticalLine = <rect
    ...
    height={1}
/>;
const GlyphHorizontalLine = <rect
    key={"horizontal-line"}
    fill={"currentColor"}
    stroke={"none"}
    strokeWidth={0}
...
const BodoniDigitSvgContent = observer(function BodoniDigitSvgContent({digit, size, color, left = 0, top = 0}: DigitProps<NumberPTM>) {
    profiler.trace();
...
const GlyphsComponent = {
    [FieldLayer.regular]: ({cells: [{top, left}], color = darkGreyColor, props: {glyphs}}: ConstraintProps<NumberPTM, GlyphsProps>) => <AutoSvg
        top={top + 0.5}
        left={left + 0.5}
        style={{color}}
    >
...
export const BodoniSudoku: PuzzleDefinition<NumberPTM> = {
    ...
    regions: Regions9,
    rules: () => <>
        <RulesParagraph>Just like regular Sudoku, except that the numbers have been replaced with typographic lines, curves, and serifs.</RulesParagraph>
        <RulesParagraph>The nine "digits" are at the keypad. The puzzle is on the left, you can add ink however you wish to solve it but you can't erase anything.</RulesParagraph>
    </>,
    items: [
        // S
