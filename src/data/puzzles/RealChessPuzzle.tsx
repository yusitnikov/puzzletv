...
export const RealChessPuzzle: PuzzleDefinition<ChessPTM> = {
    ...
    rules: () => (
        <>
            ...
            <RulesParagraph>
                <strong>{translate(almostNormalSudokuRulesApply)}</strong>: {translate(normalSudokuRulesForChessPieces)}
                .{" "}
                {translate({
                    [LanguageCode.en]: (
                        <>
                            For instance, there could be no other knights (regardless of color) in the top-left box, on
                            column <strong>d</strong> and on row <strong>7</strong>
                        </>
                    ),
                    [LanguageCode.ru]: (
                        <>
                            Например, в верхне-левом регионе, на линии <strong>d</strong> и на линии <strong>7</strong>{" "}
                            не может быть других коней (любого цвета)
                        </>
                    ),
                })}
            ...
            <RulesParagraph>
                ...
                <RulesSpoiler>
                    {translate({
                        [LanguageCode.en]:
                            "use colors to mark possible options to place different chess pieces, or to mark which cells are already under attack",
                        [LanguageCode.ru]:
                            "используйте цвета, чтобы пометить возможные места расположения различных фигур, или чтобы пометить какие поля уже атакованы",
                    })}
                    .
                </RulesSpoiler>
            </RulesParagraph>
        ...
        <>
            ...
            <RulesParagraph>
                ...
                {translate({
                    [LanguageCode.en]: (
                        <>
                            For instance, there could be no other bishops (<strong>regardless of color</strong>) in the
                            bottom-right box, in column <strong>f</strong> and in row <strong>3</strong>
                        </>
                    ),
                    [LanguageCode.ru]: (
                        <>
                            Например, в нижне-правом регионе, на линии <strong>f</strong> и на линии <strong>3</strong>{" "}
                            не может быть других слонов (<strong>любого цвета</strong>)
                        </>
                    ),
                    [LanguageCode.de]: (
                        <>
                            Beispielsweise dürfte kein weiterer Läufer (<strong>egal welcher Farbe</strong>) in die 2x3
                            Region oben links, in Spalte <strong>f</strong> oder in Zeile <strong>3</strong>{" "}
                            positioniert werden
                        </>
                    ),
                })}
            ...
            <RulesParagraph>
                {translate({
                    ...
                    [LanguageCode.de]:
                        "Außerhalb des Sudoku-Rätselgitters dürfen keine Schachfiguren positioniert werden",
                })}
                .{" "}
                {translate({
                    [LanguageCode.en]:
                        "The chess piece that moves to do the mate in one can't escape the sudoku grid either (while doing the move)",
                    [LanguageCode.ru]:
                        "Шахматная фигура, которая ходит для того, чтобы поставить мат в один ход, тоже не может выйти за пределы поля судоку для этого",
                    [LanguageCode.de]:
                        "Die Figur, die den Zug zur jeweiligen Gewinnstellung ausführt, darf den 6x6 Bereich in der Mitte ebenfalls niemals verlassen",
                })}
                .{" "}
                {translate({
                    [LanguageCode.en]:
                        "The kings can move outside the sudoku grid, though (it's not a mate if the king has a legal move outside the sudoku grid)",
                    [LanguageCode.ru]:
                        "Однако, короли могут выйти за пределы поля судоку (это не мат, если у короля есть ход за пределы поля судоку)",
                    [LanguageCode.de]:
                        "Der König kann als einzige Figur den 6x6 Bereich verlassen (solange der König also noch einen erlaubten Zug auf ein Feld außerhalb des 6x6 Bereichs machen kann, ist es kein Matt)",
                })}
                .
            </RulesParagraph>
        </>
    ...
    rules: () => (
        <>
            ...
            <RulesParagraph>
                {translate({
                    [LanguageCode.en]:
                        "Chess pieces (white or black) cannot repeat in rows, columns, boxes and diagonally",
                    [LanguageCode.ru]:
                        "Шахматные фигуры (белые или черные) не могут повторяться на каждой линии, в каждом регионе, огражденном жирными линиями, и диагонально",
                    [LanguageCode.de]:
                        "Schachfiguren (weiß oder schwarz) dürfen sich nicht in Reihen, Spalten, Kästchen und diagonal wiederholen",
                })}
                .
            </RulesParagraph>
            <RulesParagraph>{translate(mateInOne)}.</RulesParagraph>
