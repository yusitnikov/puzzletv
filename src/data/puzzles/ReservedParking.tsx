...
const hasParkedCar = (
    ...
    return cars.some(({ boundingRect: { top, left, width, height } }, index) => {
        ...
        return (
            ...
            top + Math.round(offset.top) === expected.top &&
            left + Math.round(offset.left) === expected.left
        );
    });
};
...
export const ReservedParking: PuzzleDefinitionLoader<ReservedParkingPTM> = {
    loadPuzzle: () => {
        const {
            ...
            resultChecker,
            ...base
        }: Omit<PuzzleDefinition<ReservedParkingPTM>, "slug"> = FPuzzles.loadPuzzle({
                ...
                getReactions(context): IReactionDisposer[] {
                    return [
                        reaction(
                            () => {
                                const {
                                    puzzle: { items: itemsFn, extension },
                                    allInitialDigits,
                                } = context;
                                ...
                                for (const [
                                    ...
                                    for (const { top, left, digit } of newInitialDigitsCandidates) {
                                        ...
                                        if (
                                            centerTop >= offsetTop &&
                                            centerTop <= offsetBottom &&
                                            centerLeft >= offsetLeft &&
                                            centerLeft <= offsetRight
                                        ) {
                        ...
                        if (item.cells.length === 2) {
                            ...
                            return {
                                ...
                                component: parkedRedCar
                                    ? {}
                                    : Object.fromEntries(
                                          Object.entries(item.component ?? {}).map(([layer, Component]) => [
                                              layer,
                                              (props) => (
                                                  <AutoSvg top={0} left={0} width={6.45} height={6} clip={true}>
                                                      <Component {...props} />
                                                  </AutoSvg>
                                              ),
                                          ]),
